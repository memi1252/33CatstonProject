	...
	SubShader
	{
		...
		Pass
		{
			...
			Varyings vert(Attributes input)
			{
				...
				output.uv = input.uv * _TextureSample_ST.xy + _TextureSample_ST.zw;
				return output;
			}

			half4 frag(Varyings input) : SV_Target
			{
				half4 texColor = SAMPLE_TEXTURE2D(_TextureSample, sampler_TextureSample, input.uv);

				half3 emission = (input.color.rgb * _ExtraGlow) * _Tint.rgb * texColor.rgb;
				half alpha = input.color.a * texColor.a;
